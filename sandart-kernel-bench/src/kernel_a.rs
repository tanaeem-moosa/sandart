...
pub fn census_pass(state: &mut State, scratch: &mut Scratch) -> crate::census::Census {
    ...
    for si in 0..scratch.spans.len() {
        ...
        for e in 0..n_edges {
            ...
            if scratch.candidate[e] != 0.0 {
                census.edges_nonzero_candidate += 1;
                if is_subnormal(scratch.candidate[e]) {
        ...
        for i in 0..n_data {
            ...
            if is_subnormal(out_flow) || is_subnormal(in_flow) {
