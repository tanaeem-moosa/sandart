...
/// Samples a closed outline authored as cubic Bezier segments in a 200x200, y-down drawing box
/// (the way a vector editor lays it out), then centres and scales it into the table. A straight
/// run is a cubic with its control points on the line.
fn sample_bezier_outline(start: [f32; 2], segments: &[[f32; 6]], samples_per_segment: usize) -> Vec<Vec2> {
    let mut path = Vec::with_capacity(1 + segments.len() * samples_per_segment);
    let mut p0 = Vec2::new(start[0], -start[1]);
    path.push(p0);
    for s in segments {
        let p1 = Vec2::new(s[0], -s[1]);
        let p2 = Vec2::new(s[2], -s[3]);
        let p3 = Vec2::new(s[4], -s[5]);
    ...
    center_and_scale_path(&mut path, 0.86);
    path
}

/// Side-view sauropod: long neck, rounded back, sweeping tail, four legs.
pub fn generate_dinosaur() -> Vec<Vec2> {
    let start = [2.0, 112.0];
    let segments: &[[f32; 6]] = &[
        [24.0, 116.0, 48.0, 84.0, 76.0, 72.0], // tail top sweeping up to hip
        [100.0, 62.0, 126.0, 64.0, 142.0, 74.0], // rounded back
        [150.0, 79.0, 152.0, 66.0, 154.0, 52.0], // shoulder into neck
        [158.0, 36.0, 164.0, 24.0, 176.0, 22.0], // neck up to back of head
        [188.0, 20.0, 198.0, 26.0, 197.0, 33.0], // skull to snout
        [196.0, 39.0, 186.0, 40.0, 178.0, 39.0], // mouth line
        [170.0, 40.0, 168.0, 48.0, 166.0, 58.0], // throat
        [162.0, 78.0, 160.0, 94.0, 156.0, 104.0], // front of neck to chest
        [155.0, 118.0, 156.0, 132.0, 156.0, 142.0], // near front leg, front edge
        [156.0, 147.0, 142.0, 147.0, 142.0, 142.0], // rounded foot
        [142.0, 134.0, 141.0, 126.0, 139.0, 122.0], // leg back edge
        [138.0, 128.0, 137.0, 136.0, 136.0, 142.0], // far front leg front edge
        [136.0, 146.0, 126.0, 146.0, 126.0, 142.0], // far foot
        [126.0, 134.0, 125.0, 124.0, 122.0, 116.0], // far leg back up to belly
        [112.0, 122.0, 98.0, 122.0, 90.0, 118.0], // belly
        [90.0, 126.0, 91.0, 134.0, 91.0, 142.0], // near hind leg front
        [91.0, 147.0, 76.0, 147.0, 76.0, 142.0], // foot
        [76.0, 136.0, 75.0, 130.0, 74.0, 126.0], // leg back
        [72.0, 132.0, 71.0, 137.0, 71.0, 142.0], // far hind leg
        [71.0, 146.0, 60.0, 146.0, 60.0, 142.0], // far foot
        [60.0, 130.0, 60.0, 118.0, 58.0, 110.0], // far leg back up to rump
        [40.0, 118.0, 20.0, 124.0, 2.0, 112.0],
    ];
    sample_bezier_outline(start, segments, 24)
}

/// Side-view unicorn: horn, ear, flowing mane and two-strand tail, four legs.
pub fn generate_unicorn() -> Vec<Vec2> {
    let start = [184.0, 0.0];
    let segments: &[[f32; 6]] = &[
        [178.0, 12.0, 170.0, 24.0, 164.0, 32.0], // horn back edge to forehead
        [160.0, 28.0, 158.0, 22.0, 156.0, 14.0], // ear front
        [150.0, 20.0, 148.0, 28.0, 150.0, 34.0], // ear back
        [140.0, 34.0, 132.0, 40.0, 132.0, 50.0],
        [138.0, 50.0, 140.0, 54.0, 138.0, 58.0],
        [128.0, 56.0, 122.0, 62.0, 122.0, 70.0],
        [128.0, 70.0, 130.0, 74.0, 128.0, 78.0],
        [118.0, 76.0, 112.0, 80.0, 112.0, 86.0],
        [116.0, 86.0, 118.0, 88.0, 116.0, 92.0],
        [96.0, 94.0, 76.0, 90.0, 62.0, 92.0], // back to rump
        [46.0, 94.0, 36.0, 102.0, 34.0, 114.0],
        [32.0, 128.0, 38.0, 138.0, 24.0, 152.0],
        [42.0, 150.0, 50.0, 138.0, 48.0, 126.0],
        [48.0, 138.0, 46.0, 146.0, 40.0, 154.0],
        [56.0, 148.0, 58.0, 132.0, 56.0, 120.0],
        [56.0, 112.0, 58.0, 106.0, 60.0, 104.0], // hind legs
        [58.0, 116.0, 64.0, 126.0, 64.0, 136.0], // thigh back to hock
        [64.0, 146.0, 62.0, 156.0, 62.0, 166.0], // cannon
        [62.0, 170.0, 74.0, 170.0, 74.0, 166.0], // hoof
        [74.0, 156.0, 74.0, 146.0, 74.0, 138.0],
        [75.0, 146.0, 76.0, 156.0, 76.0, 166.0], // far hind leg
        [76.0, 170.0, 88.0, 170.0, 88.0, 166.0],
        [88.0, 154.0, 86.0, 136.0, 90.0, 124.0], // up to belly
        [104.0, 128.0, 118.0, 128.0, 130.0, 124.0], // belly
        [132.0, 138.0, 132.0, 152.0, 132.0, 166.0],
        [132.0, 170.0, 144.0, 170.0, 144.0, 166.0],
        [144.0, 156.0, 143.0, 146.0, 142.0, 138.0],
        [143.0, 146.0, 146.0, 156.0, 146.0, 166.0], // far front leg
        [146.0, 170.0, 158.0, 170.0, 158.0, 166.0],
        [158.0, 150.0, 156.0, 132.0, 154.0, 118.0], // front leg up to chest
        [168.0, 108.0, 158.0, 84.0, 166.0, 68.0], // chest to throat
        [170.0, 68.0, 178.0, 72.0, 184.0, 76.0], // jaw
        [190.0, 80.0, 198.0, 78.0, 197.0, 70.0], // muzzle
        [194.0, 62.0, 184.0, 50.0, 174.0, 38.0], // face
        [176.0, 26.0, 180.0, 12.0, 184.0, 0.0],
    ];
    sample_bezier_outline(start, segments, 24)
}
